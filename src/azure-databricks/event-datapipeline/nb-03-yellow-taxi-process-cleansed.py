...
dbutils.widgets.text("folderPath", "/data/cleansed/yellow_taxi")
...
zoneSummaryDf = ( cleansedDf
                 ...
                 .agg(sum("passenger_count").cast("Int").alias("total_passenger_count"),
                     ...
                     round(mean("congestion_surcharge"),2).alias("avg_congestion_surcharge"),
                     round(mean("improvement_surcharge"),2).alias("avg_improvement_surcharge"),
                     round(mean("total_amount"),2).alias("avg_total_amount"),                   
                    ).withColumn("avg_ehail_fee", lit(0.0))
                     .withColumn("taxi_type", lit("Yellow"))
            )

# COMMAND ----------

# dbutils.fs.rm("/mnt" + curatedFolderPath+ "/vendor_summary", recurse=True)
...
publishEvent(eventSubject=dbutils.widgets.get("subject"), 
             eventType="NYCTaxi.YellowTaxi.TripData.ZoneSummaryCurated", 
